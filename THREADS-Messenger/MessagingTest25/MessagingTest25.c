...
int MessagingEntryPoint(void* pArgs)
{
	...
	console_output(FALSE, "\n%s: trying to create too many mailboxes", testName);
	...
	for (i = 0; i < MAXMBOX - 5; i++)
	{
		...
		if (mbox_id < 0)
		{
			console_output(FALSE, "\n%s: mailbox_create returned id less than zero, id = %d", testName, mbox_id);
		}
		else
		{
			if (i >= 20 && i < 30)
	...
	for (i = 0; i < 14; i++)
	{
		...
		{
			console_output(FALSE, "\n%s: mailbox_create returned id = %d", testName, mbox_id);
		}
	}
	console_output(FALSE, "\n");
	k_exit(0);
