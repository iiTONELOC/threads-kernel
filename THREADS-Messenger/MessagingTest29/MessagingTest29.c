...
int MessagingEntryPoint(void* pArgs)
{
	...
	/* To MB-8 Send 0, Receive 0 , FREE BOX*/
	...
	int result = mailbox_send(mailboxId, NULL, 0, FALSE);

	if (result == -1)
	{

		console_output(FALSE, "%s: mailbox_send returned -1: SUCCESS\n", testName);
	}
	else
	{
		console_output(FALSE, "%s: mailbox_send returned %d: FAIL\n", testName, result);
	}
